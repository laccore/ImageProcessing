...
For each horizontally-oriented core image processed, the Converter:

1. Adds a ruler to the bottom of the image
2. Creates a TIFF of the resulting image
3. Creates a JPEG of the resulting image, reducing file size by a factor of ~20 with minimal loss of image quality
4. Creates a vertically-oriented, reduced-size JPEG suitable for Initial Core Description (ICD).

The Converter was developed to replace a collection of Photoshop scripts. It is free, written in [Python](https://python.org),
and leans heavily on the [Pillow](https://python-pillow.org/) imaging library. Source code is freely available
on [GitHub](https://github.com/laccore/ImageProcessing).


## Preparing to Use the Converter

The Converter assumes images to be processed are horizontally oriented, with the core top at the left of the image.
...
editing tool like Photoshop or GIMP to resize the provided ruler as needed. For instance, if you need a 254 DPI
...
The amount to remove is indicated in inches, which is converted to pixels based on the provided Image and Ruler DPI.
...
## That's It!
If you have any questions, comments, or discover bugs, please [contact us](https://csdco.umn.edu/forms/contact-csdco)!
</body>